 ...
 * Class TIVWP_Updater
 */
// Avoid multiple class instances warning.
...
class TIVWP_Updater {
	...
	protected function _migration() {
		...
		// Migrate instance.
		if ( $_inst = get_option( $prefix . '_inst' ) ) {
			$this->instance = $_inst;
		}

		// Migrate license and email (serialized in `_data`).
		$_data = get_option( $prefix . '_data' );
		if ( ! $this->licence_key && ! empty( $_data[ $prefix . '_api' ] ) ) {
			$this->licence_key = $_data[ $prefix . '_api' ];
		}
		if ( ! $this->email && ! empty( $_data[ $prefix . '_activation_email' ] ) ) {
			$this->email = $_data[ $prefix . '_activation_email' ];
		}
